...
final class Vigsync_Plugin {
	...
	private function __construct() {
		...
		// Bloqueig de login als subsites (auto-gated per precondicions; fail-open).
		new Vigsync_Login_Guard();
	 ...
	 *
	 * Si l'esquema ja no es reconeix, desactiva només el bloqueig de login
	 * (fail-open) per no arriscar l'accés, i deixa constància.
	 *
	...
	public function after_update( $upgrader, $data ) {
		...
		if ( is_wp_error( $valid ) && Vigsync_Settings::get( 'login_block_enabled', false ) ) {
			Vigsync_Settings::update( array( 'login_block_enabled' => false ) );

			$email = sanitize_email( (string) Vigsync_Settings::get( 'notify_email', '' ) );
			if ( is_email( $email ) ) {
				wp_mail(
					$email,
					__( 'Vigilante Network Sync: bloqueig de login desactivat per seguretat', 'vigilante-network-sync' ),
					sprintf(
						/* translators: %s: missatge d'error de validació */
						__( "Després d'actualitzar Vigilante Network Sync, l'esquema de Vigilante no s'ha pogut validar (%s). S'ha desactivat el bloqueig de login per precaució. Revisa la configuració.", 'vigilante-network-sync' ),
